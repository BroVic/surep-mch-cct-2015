...
#Loading the dataset ----------------------------------
#check/set working directory using getwd()/setwd()
#clear the workspace using rm(list=ls())
data <- read.csv('surveydata.csv', stringsAsFactors = FALSE) #use appropriate function if accessing from a different location
dim(data) #confirm 314 observations with 65 variables
attributes(data)
str(data) #check dataset

ls() #only "data" should be listed
...
detach(package:Kendall, unload = TRUE)
detach(package:ggplot2, unload = TRUE)
detach(package:Formula, unload = TRUE)
unloadNamespace("latticeExtra")
...
unloadNamespace("foreign")
unloadNamespace("nnet")
unloadNamespace("boot")
unloadNamespace("digest")
unloadNamespace("acepack")
unloadNamespace("proto")
loadedNamespaces()
